...
void You_Find_Road();
int start_point();  // char*가 아닌 int 반환
int end_point();    // char*가 아닌 int 반환  
int Transport();
void T_R_Result(int start, int end);  // 매개변수 추가
void save_recent_path(int start, int finish, int hour, double minute);  // 매개변수 타입 수정
...
void You_Find_Road()
{
	int start;   // char*에서 int로 변경
	int finish;  // char*에서 int로 변경
...
int start_point()
{
	char start_reg[50];  // 크기 명시 및 포인터에서 배열로 변경
	int i;
	int start_pos = -1;
	...
	if (start_pos == -1)
	{
		...
		gotoxy(52, 26);
		printf("다시 입력해주십시오.\n");
		return start_point();  // 재귀 호출 결과를 반환
	}
	else
	{
		gotoxy(40, 25);
...
int end_point()
{
	char finish_reg[50];  // 크기 명시 및 포인터에서 배열로 변경
	int i;
	int finish_pos = -1;
